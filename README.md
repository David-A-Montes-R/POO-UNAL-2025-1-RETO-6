...
class Point:
    def __init__(self, x: float, y: float):
        ...
        if self._x is None or self._y is None:
            raise ValueError("al punto le falta una o varias coordenadas") #esto por alguna razón no se refleja en la ejecución
        if not isinstance(self._x,(int, float)) or not isinstance(self._y,(int, float)):
            raise TypeError("el punto no posee coordenadas válidas")
...
class Line:
    def __init__(self, point_start: "Point", point_end: "Point"):
        ...
        if not isinstance(self._point_start,Point) or not isinstance(self._point_end,Point):
            raise TypeError("Los puntos de inicio y fin NO son válidos")
        self.length = (float(self._point_end._x - self._point_start._x)**2 +
                  float(self._point_end._y - self._point_start._y)**2)**(1/2)
        if self.length == 0:
            raise ValueError("la longitud de la línea no puede ser 0")
...
class Shape:
    def __init__(self, is_regular: bool, *Lines: "Line"):
        some_edges = []
        self.lines = Lines
        if len(self.lines) < 3 or self.lines == None:
            raise ValueError("la figura debe tener por lo menos 3 líneas")
...
class Scalene(Triangle):
    def __init__(self, is_regular, *Lines):
        ...
        if not (comp_1 and comp_2 and comp_3):
            raise TypeError("este no es un triángulo escaleno, introduzca un triángulo válido")
...
class Isosceles(Triangle):
    def __init__(self, is_regular, line_base : "Line", third_point: "Point"):
        ...
        self.lines = [line_base, Line(line_base._point_start,third_point), #igual toca meter 3 puntos para el triángulo pero por lo menos no 3 líneas
                      Line(line_base._point_end,third_point)]
        #se verifica que el triángulo sea Isosceles
        comp_1 = self.lines[0].length == self.lines[1].length
        comp_2 = self.lines[1].length == self.lines[2].length
        comp_3 = self.lines[2].length == self.lines[0].length 
        if not (comp_1 or comp_2 or comp_3):
            raise TypeError("este no es un triángulo isosceles, introduzca un triángulo válido")
...
class Equilateral(Triangle):
    def __init__(self, is_regular, line_base : "Line", third_point: "Point"):
        ...
        if not (comp_1 and comp_2 and comp_3): #se verifica que el triángulo sea equilatero
            raise TypeError("este no es un triángulo equilatero, introduzca un triángulo válido")
        else: super().__init__(is_regular, *self.lines)
    def get_line_base(self):
        return self._line_base
    def set_line_base(self, new_line_base: "Line"):
...
class TriRectangle(Triangle): #genera un triángulo rectángulo con sus catetos
    def __init__(self, is_regular, *Lines):
        ...
        if not 0 in cosines: #se verifica que el triángulo sea rectángulo
            raise TypeError("este no es un triángulo rectángulo, introduzca un triángulo válido")
        else: pass 
...
class Rectangle(Shape):
    def __init__(self, is_regular, *Lines):
        ...
        if not len(self.lines) == 4:
            ...
            print(self.lines)
            raise TypeError("estas líneas no pueden formar un rectángulo")
